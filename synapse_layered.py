...
def connect_layers_excitory(G1, G2, connection_probability, net, synapses):
    ...
    #         w -= wIncrement * condition_LTP_reverse
    #         w -= wIncrement * condition_Remove_Mg * condition_LTD * 5
...
def generate_layers(G1, G1_spikeMOnitor ,numberOfLayersNeed):
    ...
    for count in range(numberOfLayersNeed):
        ...
        net.add(layers[-1])
        (net, synapses) = connect_layers_excitory(layers[count], layers[count+1], 0.1, net, synapses)
